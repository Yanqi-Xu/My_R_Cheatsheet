...
Hi! Welcome to my Data Journalism Cleaning R Cheatsheet for cleaning and wrangling data. Just like the [R cheatsheet resources][10] of package-based cheatsheet, Journalists have put out cheatsheets before, like [MaryJo Webster's R Cheatsheet][03]. Hat tip to her and her [amazing collection of data journalism resources][04]!
...
```{r echo=FALSE}
kable(
x = tibble(what_it_does = pull(My_R_Cheatsheet[cutoff[length(cutoff)]+1:length(My_R_Cheatsheet),1]),
       on_the_screen = pull(My_R_Cheatsheet[cutoff[length(cutoff)]+1:length(My_R_Cheatsheet),2]),
      you_will_type = pull(My_R_Cheatsheet[cutoff[length(cutoff)]+1:length(My_R_Cheatsheet),3]))) %>% 
...
This cheatsheet was originally a [spreadsheet][13], and you can learn about how I turned it into a rendered html page with the package `knitr` in the Rmd.
...
Feel free to use the cheatsheet whatever way you want. If there's any R function you wish to add that could be helpful for data cleaning, please fill out this [google form][12] and I'll add them to this thing.